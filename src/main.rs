...
use solana_sdk::{
    program_pack::Pack, pubkey::Pubkey, signature::Keypair,
...
fn main() {
    ...
    if let Ok(airdrop_signature) = request_air_drop(&rpc_client, &sender.pubkey(), 1.0) {
        ...
        for result in rdr.records() {
            ...
            let transfer_amount = 0.0005;
            let receiver = Pubkey::from_str(&record[1]).expect("Failed to create Pubkey from string");
